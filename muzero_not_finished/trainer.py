from utils import value_reward_transfomer, discrete_transform
    ...
    def sample(self):
        ...
        for history, targets in sample_games:
            ...
            for i in range(len(values[index:index+self.config.num_unroll_steps])):
                ...
                for j in range(1, self.config.td_steps + 1):
                    if i + j + self.config.td_steps - 1 > len(rewards[index:]):
                        value += 0
                    else:
                        value += rewards[index:][i+j] * (self.config.discount ** (j-1))
...
class Trainer:
    ...
    def update_weights(self, model, replay_buffer):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            for data in samples:
                ...
                for i in range(self.config.num_unroll_steps):
                    ...
                    value_target = discrete_transform(
                        value_reward_transfomer(data[1][3][i]),
                        self.config.discrete_support_size,
                        self.config.discrete_floor,
                        self.config.discrete_ceiling
                    )

                    reward_target = discrete_transform(
                        value_reward_transfomer(data[1][2][i]),
                        self.config.discrete_support_size,
                        self.config.discrete_floor,
                        self.config.discrete_ceiling
                    )
                    ...
                    ) + self.config.value_loss_discount * tf.keras.losses.categorical_crossentropy(
                        y_pred=value_prob,
